...
describe('createMatchViewModel', () => {
  ...
  it('does not show the crown when highest points are tied or belong only to a normal bot', () => {
    const base = createPlayingSessionState();
    const tiedViewModel = createMatchViewModel({
      ...base,
      roomSnapshot: {
        ...base.roomSnapshot!,
        payload: {
    ...
    const botOnlyViewModel = createMatchViewModel({
      ...base,
      roomSnapshot: {
        ...base.roomSnapshot!,
        payload: {
    ...
    expect(tiedViewModel.players.map((player) => player.title)).not.toContain('👑');
    expect(botOnlyViewModel.players.map((player) => player.title)).not.toContain('👑');
  });
