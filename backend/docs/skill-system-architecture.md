...
- The last production `RoomState <-> Value` bridges inside standard execution have now been removed as well. `rules/standard/actions.rs`, `rules/standard/flow.rs`, `rules/standard/win.rs`, and `rules/skills/mod.rs` now execute their typed room-state entrypoints natively; the only remaining whole-room conversion in `backend/src` is the `#[cfg(test)]` wrapper in `core/engine/flow.rs`.

### 16.2 Still Transitional / Not Yet Finished

- The typed state is now the single runtime write-time source of truth inside `backend/src`, and that is now true for the hot standard/skill execution paths as well as the app/runtime shell. What remains transitional is almost entirely the outer storage/protocol boundary: room persistence, command intake, and outbound events/messages still rely on JSON compatibility layers.
- The write path has moved fully into typed state inside the backend core, and the previously remaining standard/skill `RoomState <-> Value` execution bridges have been retired. The main remaining dependency is no longer an internal execution-step detour; it is the parse/serialize bridge between typed state and the still-JSON room/message schema at the outer boundary.
...
- The biggest remaining transition layer is no longer `mahjong.rs`. That file has been pushed out of the production runtime path; the remaining transition now lives mostly inside typed-to-JSON bridges that still exist inside some standard-rule/core-engine entrypoints and in outward protocol/persistence serialization.
- The most visible remaining transition surfaces are now outward schema adapters rather than execution internals. `RoomRuntime` is typed, `app/*` no longer routes through `mahjong.rs`, standard continue-action / restart progression no longer needs a typed-to-JSON detour, bot/timeout automation no longer needs a room-wide detour either, and the remaining whole-room conversion inside `core/engine/flow.rs` is test-only; what remains is mainly websocket/server payload shaping, stored room JSON, and the still-JSON command/snapshot/message contracts.
