...
- The typed state is now the single runtime write-time source of truth inside `backend/src`. All active write paths pass through typed `RoomState` mutation before serializing back out. What remains transitional is the storage/protocol boundary: room persistence, command intake, and outbound events/messages still rely on legacy-shaped JSON compatibility layers.
- The write path has moved fully into typed state inside the backend core, but the backend is not yet free of legacy compatibility concerns. Runtime room state no longer flows through a mutation DSL, planner outputs no longer expand to compatibility mutations, and hot standard/skill write paths all mutate typed structures directly. The main remaining dependency is the repeated parse/serialize bridge between typed state and the existing legacy room/message schema.
- The engine split has moved past the original `mod.rs` catch-all layout. `core/engine/command.rs`, `flow.rs`, `validation.rs`, `planner.rs`, and `reducer.rs` now exist, and legacy command parsing plus local command execution dispatch have been extracted out of `core/engine/mod.rs` and `mahjong.rs`. Standard claim-window / rob-kong timeout handling and hu-action hinting have also started consuming typed `PendingAction` instead of reparsing legacy JSON in-place. Discard / claim / flower / self-kong / hu / exhaustive-draw settlement execution now produce typed `EngineOutput` directly, but some timeout automation paths still retain legacy-message-first adaptation.
...
- `mahjong.rs` still exists as a compatibility facade. It delegates far more to typed engine/state/projection code than before, and room snapshot / match-result shaping have both shrunk there, but some fallback JSON-building paths and protocol adapters still sit there as an outward-facing layer.
- The most visible remaining legacy anchors are now the outer room/message schema adapters themselves. Most compatibility conversion aliases have already been removed from active core state types, and `GameEvent::LegacyRoundEvent`-style outward transport has already been removed as well. What remains is primarily room persistence plus the outward protocol schema itself: websocket/server payload contracts, room snapshots, and outbound messages still serialize to the existing legacy JSON shape even when the data is now projected from typed state.
- The most visible remaining legacy anchors are now the outer room/message schema adapters themselves. The `app` layer now mostly reads and updates rooms through typed state internally, and continue-action state now serializes in typed nested form internally, but stored room payloads and client-facing payloads still preserve the broader legacy JSON contract at the boundary.
- The most visible remaining legacy anchors are now the outer room/message schema adapters themselves. Compatibility conversion aliases still exist in several state modules, but active runtime code no longer depends on the old names, and `GameEvent::LegacyRoundEvent`-style outward transport has already been removed. What remains is primarily the outward protocol/schema contract itself: websocket/server payloads, room snapshots, and stored room payloads are now produced from typed state more consistently, but they still preserve the existing legacy JSON shape for compatibility.
- The most visible remaining legacy anchors are now the outer room/message schema adapters themselves. Compatibility conversion aliases still exist in several state modules, but active runtime code no longer depends on the old names, and `GameEvent::LegacyRoundEvent`-style outward transport has already been removed. What remains is primarily the outward protocol/schema contract itself: websocket/server payloads, room snapshots, and stored room payloads are now produced from typed state and typed protocol helpers more consistently, but they still preserve the existing legacy JSON shape for compatibility.
...
- The biggest remaining architecture debt is no longer "can skills be integrated?", because that milestone has already been crossed. The main debt is now "can the remaining legacy mutation/runtime compatibility layers be pulled behind the new typed boundaries?"
...
- This is the right point to stop treating skills as the main unknown and instead finish the typed reducer / scoring / facade cleanup so the skill-enabled architecture becomes the default architecture rather than a compatibility layer around legacy JSON mutation.

## 17. Recommended Next Steps

### 17.1 First Priority: Replace Legacy JSON Mutation As The Primary Write Path
...
- make `RoomState` / `RoundState` the actual source of truth at runtime
- remove the largest remaining architectural mismatch between the new typed boundaries and the still-legacy mutation pipeline
