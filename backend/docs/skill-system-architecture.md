...
- Planner output has started crossing the same boundary for discard flow. `plan_discard_action` no longer returns raw `discard_mutations` / `followup_mutations` vectors as its primary shape; it now exposes a typed discard continuation model and keeps legacy mutation expansion as an adapter method at the edge.
...
- The engine split has moved past the original `mod.rs` catch-all layout. `core/engine/command.rs`, `flow.rs`, `validation.rs`, `planner.rs`, and `reducer.rs` now exist, and legacy command parsing plus local command execution dispatch have been extracted out of `core/engine/mod.rs` and `mahjong.rs`. Standard claim-window / rob-kong timeout handling and hu-action hinting have also started consuming typed `PendingAction` instead of reparsing legacy JSON in-place. Discard and claim command execution now produce typed `EngineOutput` directly, but the remaining timeout / flower / self-kong / hu branches are still partly adapted through legacy-message-first helpers.
- Settlement is no longer the main typed-state blocker, and skill tracker state has also crossed into typed structures. The remaining compatibility seams are now concentrated more in the mutation/result pipeline and outward-facing legacy serialization than in tracker/settlement internals.
