...
class GameService:
    ...
    def _deserialize_room(self, payload: dict) -> RoomState:
        ...
        for seat_payload in payload.get("seats", []):
            ...
            legacy_ai_seat = seat_payload.get("seat_type") == "ai"
            ...
            is_bot = seat_payload.get("is_bot", False) or legacy_ai_seat
            if legacy_ai_seat:
                connected = True
                ready = True
            room.seats[seat_index] = SeatReservation(
            ...
            if room.seats[seat_index].is_bot:
                self._ensure_bot_profile(room.seats[seat_index])
