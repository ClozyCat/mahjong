...
async fn spectator_non_player_request_is_reviewed_by_target_player() -> Result<()> {
    ...
    let viewer_requests = app
        ...
        .await?;
    assert_eq!(viewer_requests.status(), StatusCode::OK);
    let viewer_requests_body = json_response(viewer_requests).await;
    assert_eq!(viewer_requests_body.as_array().map(Vec::len), Some(1));
    assert_eq!(viewer_requests_body[0]["id"], request_id);
    assert_eq!(viewer_requests_body[0]["status"], "approved");
    Ok(())
}

#[tokio::test(flavor = "current_thread")]
async fn spectator_request_targeting_special_bot_is_rejected() -> Result<()> {
    let (app, worker, state) = test_app().await?;
    server::seed_special_bot_users(&state).await?;
    let bot = worker
        .find_user_by_identifier("bot_schubert")
        .await?
        .expect("special bot should be seeded");
    let (owner_token, owner_id) = register_user(&app, &worker, "INVITE100103", "Owner").await?;
    let (viewer_token, _viewer_id) = register_user(&app, &worker, "INVITE100104", "Viewer").await?;
    ...
    let request = app
        ...
        .oneshot(authed_json_request(
            ...
            &format!("/api/tables/{table_code}/spectator-requests"),
            &viewer_token,
            json!({ "target_user_id": bot.user_id }),
        ))
        .await?;

    assert_eq!(request.status(), StatusCode::CONFLICT);
    let body = json_response(request).await;
    assert_eq!(body["detail"], "special_bot_cannot_be_watched");
    Ok(())
}
...
async fn spectator_player_cannot_request_to_watch_another_table_while_playing() -> Result<()> {
    ...
    let table_b = create_table(&app, &owner_b_token, 1).await?;
    set_table_phase(&state, &worker, &table_b, "playing").await?;

    let request_response = app
        .clone()
        .oneshot(authed_json_request(
            Method::POST,
            &format!("/api/tables/{table_b}/spectator-requests"),
            &guest_token,
            json!({}),
        ))
        .await?;
