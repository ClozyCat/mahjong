...
from league_config import build_eval_config, build_trajectory_configs
...
def test_trajectory_config_uses_learner_for_subject_and_three_sampled_opponents() -> None:
    ...
    config = build_trajectory_configs(pool, matches=2, seed=7, max_actions=20)[0]
...
def test_awr_wrapper_uses_v2_sft_checkpoint_and_rollout_override() -> None:
    ...
    assert "--output \"$trajSummary\"" in script
