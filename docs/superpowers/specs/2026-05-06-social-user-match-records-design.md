...
- 观战必须由被申请观战的玩家审批，且当前牌局内玩家不能观战本牌局。
...
- 积分允许为负数。
- bot 不写入用户积分事件。
- 如果结算时房间内存在真正的 bot 座位，则该局不影响任何玩家的局外积分：不写入该局 `user_point_events`、不更新 `users.points`，并将 `round_player_results.point_delta` 记为 `0`。
- `bot 代打` 的真人座位仍视为人类座位，不按 bot 处理；这里只排除独立 bot 座位。
...
3. 服务端创建 `spectator_requests`，通过 `/ws/me` 推送给被申请观战玩家的所有在线设备。
4. 被申请观战玩家同意后，申请者进入观战 WebSocket 流程。
5. 被申请观战玩家拒绝或申请超时，申请者收到提示。
...
- 所有连接断开前座位不进入断线状态。
- 纯真人牌局结算按锁定倍数写积分事件。
- 结算时若存在独立 bot 座位，则该局不影响玩家局外积分；bot 代打的人类座位仍按真人处理。
...
- 玩家只能通过邀请进入牌局。
- 多设备登录同一账号能同时操作同一座位。
- 纯真人牌局每局结算后玩家积分按分差和倍数变化，番种统计更新。
- 含独立 bot 座位的结算局只归档记录与番种，不影响玩家局外积分。
- 用户资料、排行榜、牌局记录均公开可见。
- 观战默认启用，且必须经过被申请观战的玩家同意。
