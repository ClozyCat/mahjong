...
Expose neural logits/value from `backend/src/bot/neural.rs` and fill `ArenaTrajectoryRow.log_prob`/`value` in `trajectory_row_from_trace` when the policy mode is `Neural` or `Hybrid` with a model path. For heuristic-only rows, keep zeros and mark them unsuitable for PPO unless recomputed from a checkpoint.
...
- PPO may still be off-policy if trajectories are generated by heuristic/hybrid deterministic choices. Control: use `--recompute-old-policy-stats` only for warm-start smoke, then add neural-backed stochastic rollout before judging strength.
